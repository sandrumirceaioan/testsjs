...
let lastDayLastMonth =  moment().subtract(1, 'month').endOf('month').format('YYYY-MM-DD');
let companiesData = [];
let individualData = [];

let getStats = function(user,start,end,ind){
	if (!end) {
		end = lastDayLastMonth;
	} 
	if (start < firstDayLastMonth) {
		start = firstDayLastMonth;
	} 
	let payout = 0;
	for (let i=0; i<stats.length; i++) {
		if (user === stats[i].username && stats[i].date >= start && stats[i].date <= end) {
			payout += stats[i].payout;
			if (!ind) {stats.splice(i,1);}
		}
	}
	return payout;
};
		...
		if (users[i].history.length > 0) {
			...
			for (let j=0; j<users[i].history.length; j++) {
				if (companies[c].name === users[i].history[j].company && users[i].history[j].startDate < firstDay  && (!users[i].history[j].endDate || users[i].history[j].endDate >= firstDayLastMonth)) {
					let amount = getStats(users[i].username,users[i].history[j].startDate,users[i].history[j].endDate);
					companiesData.push({company:companies[c].name,user: users[i].username, payout: amount});
				}
			}
		} else {
...
for (let i=0; i<users.length; i++) {
	let amount = getStats(users[i].username,firstDayLastMonth,lastDayLastMonth,true);
	individualData.push({user: users[i].username, payout: amount});
}

let totalCompanies = 0;
for (let i=0; i<companiesData.length; i++) {
	totalCompanies += companiesData[i].payout;
}

let totalIndividual = 0;
for (let i=0; i<individualData.length; i++) {
	totalIndividual += individualData[i].payout;
}

console.log(companiesData);
console.log(individualData);
console.log(totalStats);
console.log(totalCompanies + totalIndividual);
